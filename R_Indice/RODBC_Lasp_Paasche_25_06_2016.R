...
#En cas de changement de machine, veiller a bien corriger l'adresse server, le nom du domaine et le sqlquery!! Ensuite veiller a installer RTools et les autres librairies nécessaires.
...
#Exportation des Fichiers Excela
...
library(ggplot2)
g1 = ggplot(my_data3, aes(x = temps, y=Ind_Lasp)) + aes(colour = Flux) + geom_line() + geom_smooth() + xlab("P?riode") + ylab("Indice Global(%)") + ggtitle("Indices des prix ? l'Importation et ? l'Exportation")
ggsave("Graphe_1.pdf")
g2 = ggplot(my_data3, aes(x = Mois, y=Ind_Lasp, group=Annee)) + geom_line() + aes(colour=Annee) + facet_grid(.~Flux) + ylab("Indice Global(%)") + ggtitle("Evolution comparative des indices ? l'Importation et ? l'Exportation")
ggsave("Graphe_2.pdf")
