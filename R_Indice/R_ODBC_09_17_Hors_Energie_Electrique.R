...
res <- sqlQuery(dbconnection, "SELECT [Year], [Period],[FLOW],[HS2012] as SH4, CONVERT(NUMERIC, sum([VALEURINSAE])) as VALEUR, CONVERT(NUMERIC, sum([POIDNET])) as POIDSNET, CONVERT(NUMERIC, sum([QUANTITE])) as QUANTITE FROM [dbo].[BENIN_DATA_INDEX] WHERE [Year] in (2010,2011, 2012, 2013, 2014, 2016, 2017) and [Period] in ('01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12') and [TRADETYPE] in('2', 'G') and [FLOW] in ('I', 'E', 'R') and LEN([HS2012])=4 group by [Year],[Period],[FLOW],[HS2012]", as.is=T)
close(dbconnection)
...
ech<-read.xlsx("Echantillon2015x.xlsx", sheet= 2)
...
write.xlsx(x = my_data3, file = paste("Indices_Globaux_Hors_ Energie_Electrique", Sys.Date(), ".xlsx"),
           sheetName = "Indices", col.NAmes = TRUE, row.Names = FALSE)
write.xlsx(x = my_data2, file = paste("Indices_Produits_Hors_ Energie_Electrique", Sys.Date(), ".xlsx"), sheetName = "Indices", row.names = FALSE)

write.xlsx(x = test3SH, file = paste("Indices_Chapitres_Hors_Energie_Electrique", Sys.Date(), ".xlsx"),
           sheetName = "Indices", col.NAmes = TRUE, row.Names = FALSE)
write.xlsx(x = test3STA, file = paste("Indices_Statut_Hors_ Energie_Electrique", Sys.Date(), ".xlsx"),
           sheetName = "Indices", col.NAmes = TRUE, row.Names = FALSE)
# Graphique

library(ggplot2)
g1 = ggplot(my_data3, aes(x = temps, y=Ind_Lasp)) + aes(colour = Flux) + geom_line() + geom_smooth() + xlab("P?riode") + ylab("Indice Global(%)") + ggtitle("Indices des prix à l'Importation et à l'Exportation")
ggsave("Graphe_Global_Hors_ Energie_Electrique.pdf")
...
ggsave("Graphe_Comparatif_2_Hors_ Energie_Electrique.pdf")
