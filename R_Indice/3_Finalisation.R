...
E2<-E1 %>%
  ...
  group_by(HS6)%>%
  mutate(Pond0=sum(Pond10000[which(Year==2015)]))%>%
  ungroup(.)%>%
  group_by(HS2)%>%
  mutate(PondHS2=sum(Pond10000[which(Year==2015)]))%>%
  ungroup(.)%>%
  group_by(HS4)%>%
  mutate(PondHS4=sum(Pond10000[which(Year==2015)]))%>%
  left_join(.,REL_HS2SITC, by=c("HS6"="HS6"))%>%
  mutate(SITC1=substr(TRS_SITC,1,1), SITC2=substr(TRS_SITC,1,2))%>%
  ungroup(.)%>%
  group_by(SITC1)%>%
  mutate(PondSITC1=sum(Pond10000[which(Year==2015)]))%>%
  ungroup(.)%>%
  group_by(SITC2)%>%
  mutate(PondSITC2=sum(Pond10000[which(Year==2015)]))

#HS  
Exp_HS2_Lasp<- E2%>%
  group_by(Year,Period,HS2)%>%
  summarise(Export_Lasp_HS2=sum(Carli*Pond0)/mean(PondHS2), Pond=mean(PondHS2))%>%
  mutate(temps=as.Date(paste(Year, Period, 1, sep="-"), "%Y-%m-%d"))%>%
  left_join(.,DIC_HS, by=c("HS2"="Code"))
  
Exp_HS4_Lasp<- E2%>%
  group_by(Year,Period,HS4)%>%
  summarise(Export_Lasp_HS4=sum(Carli*Pond0)/mean(PondHS4), Pond=mean(PondHS4))%>%
  mutate(temps=as.Date(paste(Year, Period, 1, sep="-"), "%Y-%m-%d"))%>%
...
Exp_SITC1_Lasp<- E2%>%
  ...
  summarise(Export_Lasp_SITC1=sum(Carli*Pond0)/mean(PondSITC1), Pond=mean(PondSITC1))%>%
  mutate(temps=as.Date(paste(Year, Period, 1, sep="-"), "%Y-%m-%d"))%>%
  left_join(.,DIC_SITC, by=c("SITC1"="Code"))

Exp_SITC2_Lasp<- E2%>%
  group_by(Year,Period,SITC2)%>%
  summarise(Export_Lasp_SITC2=sum(Carli*Pond0)/mean(PondSITC2), Pond=mean(PondSITC2))%>%
  mutate(temps=as.Date(paste(Year, Period, 1, sep="-"), "%Y-%m-%d"))%>%
...
Exp_Gl_Lasp<-E2%>%
  ...
  summarize(Export_Lasp=sum(Carli*Pond0)/10000)%>%
  mutate(temps=as.Date(paste(Year, Period, 1, sep="-"), "%Y-%m-%d"))#%>%
  #filter(!is.na(temps))
...
Imp_HS2_Lasp<- I2%>%
  group_by(Year,Period,HS2)%>%
  summarise(Import_Lasp_HS2=sum(Carli*Pond0)/mean(PondHS2), Pond=mean(PondHS2))%>%
  mutate(temps=as.Date(paste(Year, Period, 1, sep="-"), "%Y-%m-%d"))%>%
  left_join(.,DIC_HS, by=c("HS2"="Code"))
...
#SITC
  Imp_SITC1_Lasp<- I2%>%
    group_by(Year,Period,SITC1)%>%
    summarise(Import_Lasp_SITC1=sum(Carli*Pond0)/mean(PondSITC1), Pond=mean(PondSITC1))%>%
    mutate(temps=as.Date(paste(Year, Period, 1, sep="-"), "%Y-%m-%d"))%>%
    left_join(.,DIC_SITC, by=c("SITC1"="Code"))
...
Imp_Gl_Lasp<-E2%>%
  group_by(Year, Period)%>%
  summarize(Import_Lasp=sum(Carli*Pond0)/10000)%>%
  mutate(temps=as.Date(paste(Year, Period, 1, sep="-"), "%Y-%m-%d"))#%>%
#filter(!is.na(temps))
